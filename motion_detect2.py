...
while True:
        ...
        #   out=subprocess.run(['/bin/bash','/home/peter/turn_mon_on.sh'])
        #   screen_on = 1
        #   syslog.syslog("turn on")
        if (len(contours) == 0) and (screen_on == 1) and (ontime < datetime.now())  :
           out=subprocess.run(['/bin/bash','/home/peter/turn_mon_off.sh'])
           screen_on = 0
           syslog.syslog("turn off")
        for contour in contours:
                if cv2.contourArea(contour) > 50:
                        #print(cv2.contourArea(contour))
                        ontime = datetime.now() + timedelta(seconds=delay_time)
                if (cv2.contourArea(contour) < 50) and (screen_on == 1) and (ontime < datetime.now()):
                        print("no")
                        out=subprocess.run(['/bin/bash','/home/peter/turn_mon_off.sh'])
                        screen_on = 0
                        syslog.syslog("turn off")
                        print("turn off")
                if (cv2.contourArea(contour) > 50) and (screen_on == 0) and (time(5) <= datetime.now().time() <= time(22)):
                        out=subprocess.run(['/bin/bash','/home/peter/turn_mon_on.sh'])
                        screen_on = 1
                        syslog.syslog("turn on")
                        print("turn on")
